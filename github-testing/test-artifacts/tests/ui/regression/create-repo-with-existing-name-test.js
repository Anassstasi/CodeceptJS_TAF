...
/**
 * [Test-Case ID:GH-0008]
 * 
 * Tests CREATE A REPO functionality.
 * Creates a repo using invalid data.
 */
Scenario(`[Test-Case ID:GH-0008] - Create a repo test| @repo @create_repo ${TEST_GROUPS.REGRESSION}`,
 async () => {
    await mainHub.click('header', 'addBtn')
    await mainHub.click('header', 'newRepoBtn')

    await createRepoPage.createNewRepo(REPO.NAME, REPO.DESCRIPTION)

    await createRepoPage.see('errorMsg')
});
